...
frisby.create('PUT missing locationName for user ' + user.email)
    .put(uri + '/locations',
         {'locationPostalCode' : tu1Home.zip })
...
frisby.create('PUT missing locationPostalCode for user ' + user.email)
    .put(uri + '/locations',
...
frisby.create('DELETE missing locationPostalCode for user ' + user.email)
    .delete(uri + '/locations',
         {'locationName' : tu1Home.name })
    .addHeader('Authorization', 'Bearer ' + user.token.access_token)
    .expectStatus(400)
...
frisby.create('DELETE Remove unknown location for user ' + user.email)
    .delete(uri + '/locations',
         {'locationName' : 'foo', 'locationPostalCode' : 'bar' })
    .addHeader('Authorization', 'Bearer ' + user.token.access_token)
    .expectStatus(404)
    .expectHeader('Content-Type', 'application/json; charset=utf-8')
    .expectJSON({'error' : 'User does not have location locationName "foo", locationPostalCode "bar"'})
    .toss()
